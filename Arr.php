 ...
 *
 * @link http://fuelphp.com/docs/classes/arr.html
 */
class Arr
{
    ...
    public static function insert(array &$original, $value, $pos)
    {
        ...
        {
            trigger_error('Position larger than number of elements in array in which to insert.', E_NOTICE);
            return false;
        }
    ...
    public static function insert_before_key(array &$original, $value, $key, $is_assoc = false)
    {
        ...
        if ($pos === false)
        {
            trigger_error('Unknown key before which to insert the new value into the array.', E_NOTICE);
            return false;
        }
    ...
    public static function insert_after_key(array &$original, $value, $key, $is_assoc = false)
    {
        ...
        if ($pos === false)
        {
            trigger_error('Unknown key after which to insert the new value into the array.', E_NOTICE);
            return false;
        }
    ...
    public static function insert_after_value(array &$original, $value, $search, $is_assoc = false)
    {
        ...
        if ($key === false)
        {
            trigger_error('Unknown value after which to insert the new value into the array.', E_NOTICE);
            return false;
        }
    ...
    public static function insert_before_value(array &$original, $value, $search, $is_assoc = false)
    {
        ...
        if ($key === false)
        {
            trigger_error('Unknown value before which to insert the new value into the array.', E_NOTICE);
            return false;
        }
    ...
    /**
     * @param array $arr
     * @param array $filters <pre>array(
     *       array("list" => array("ppgraphicart", "dsmmagicname"), "refid" => array("0002", "1389")),
     *       array("list" => "pptextart", "refid" => "0014"),
     *       array("refid" => "0014"),
     *       array("list" => "ppcollage", "refid" => array("0001")),
     *   )
     * )</pre>
     * @return array
     */
    public static function filterBy($arr, $filters) {

        if(!is_array($filters)) {
            throw new \BadMethodCallException('Parameter "filters" has to be an array');
        }

        if(Arr::is_assoc($filters)) {
        ...
        foreach ($arrIt as $sub) {
            ...
            $subIterator = $arrIt->getSubIterator();
            $subArray = $subIterator->getArrayCopy();

            foreach ($filters as $index => $filter) {

                if(count(array_intersect_key($filter, $subArray)) !== count($filter)) {
                    continue;
                }
                ...
                foreach ($filter as $filterKey => $filterValue) {
                    ...
                    if(!in_array($subArray[$filterKey], $filterValue)) {
                        $hit = false;
                        break;
                    }
                }

                if($hit && !in_array($subArray, $result)) {
    ...
    /**
     * Findet das erste Vorkommen von needle zu der Eigenschaft prop_name
     * in arr und gibt das Element zurück
     *
     * @param mixed $needle
     ...
     * @return bool|array
     */
    public static function findWhere($needle, $prop_name, $arr)
    {
        $key = array_search($needle, array_column($arr, $prop_name));
        if($key === false) {
            return false;
        }
        return $arr[$key];
